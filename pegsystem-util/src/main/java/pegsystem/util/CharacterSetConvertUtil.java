 ...
 * @Description : DB의 캐릭터셋이 US-ASCII(7비트)인 경우 한글이 깨지기 떄문에 캐릭터셋을 변경하기 위한 Util
 *              Class context-common.xml에 bean으로 등록할때 in, out 캐릭터셋을 반드시 생성자
 *              argument로 지정해야 한다.
 * 
 * @author 박진우(jwpark@pegsystem.co.kr)
 * @since 2018.04.04
 * @version 1.0
 * @see Copyright (C) PEGSYSTEM <http://www.pegsystem.co.kr>
 * 
 * 
 * @Modification Information Date Modifier Comment ---------- ----------
 *               ------------------- 2018.04.04 박진우 최초생성
 */
public class CharacterSetConvertUtil {
				...
				try {
					...
					if (str != null) {
						retMap.put(entry.getKey(),
								new String(str.toString().getBytes(OUT_CHARACTER_SET), IN_CHARACTER_SET));
					}
	...
	public String convertParameter(String str) {
		if (CHARACTER_SET_CHECK) {
			...
			try {
				if (str != null)
					retVal = new String(str.getBytes(OUT_CHARACTER_SET), IN_CHARACTER_SET);
			} catch (Exception e) {
				e.printStackTrace();
				logger.info(e.getMessage());
			}
				...
				if (entrySet != null) {
					...
					if (iter != null) {
						while (iter.hasNext()) {
							Entry<String, Object> entry = (Entry<String, Object>) iter.next();
							try {
								retMap.put(entry.getKey(), new String(
										entry.getValue().toString().getBytes(IN_CHARACTER_SET), OUT_CHARACTER_SET));
							} catch (UnsupportedEncodingException e) {
								e.printStackTrace();
								logger.info(e.getMessage());
							}
	...
	public String convertResultSet(String str) {
		if (CHARACTER_SET_CHECK) {
			...
			try {
				if (str != null)
					retVal = new String(str.getBytes(IN_CHARACTER_SET), OUT_CHARACTER_SET);
			} catch (Exception e) {
				e.printStackTrace();
				logger.info(e.getMessage());
			}
